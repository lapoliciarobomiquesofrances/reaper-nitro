package me.rickytheracc.reapernitro.modules.misc.elytrabot.utils;
...
public class AStar {
    ...
    public static ArrayList<BlockPos> addToOpen(BlockPos[] positions, ArrayList<BlockPos> checkPositions, BlockPos current, BlockPos goal, BlockPos start, ArrayList<BlockPos> open, ArrayList<BlockPos> closed) {
        ...
        outer: for (BlockPos pos : positions2) {
            if (!MiscUtil.isSolid(pos) && !closed.contains(pos)) {
                ...
                for (BlockPos check : checkPositions2) {
                    if (Modules.get().get(ElytraBotThreaded.class).botMode.get() == ElytraBotThreaded.Mode.Highway && !MiscUtil.isInRenderDistance(check)) {
                        return null;
                    }

                    if (MiscUtil.isSolid(check) || !MiscUtil.isInRenderDistance(check)) {
                        continue outer;
                    }

                    if (MiscUtil.getBlock(check) == Blocks.LAVA && Modules.get().get(ElytraBotThreaded.class).avoidLava.get()) {
                        continue outer;
                    }
