...
import me.rickytheracc.reaperplus.util.world.DamageCalculator;
...
public class BedGod extends Module {
    ...
    private DamageCalculator.BedPlacement placePos;
    private Renderers.SimpleBedRender bedRender;
    private Interactions.MineInstance trapMine, burrowMine;
    ...
    public BedGod() {
        super(ReaperPlus.R, "bed-god", "the best bed aura");
    }
...
//        }
    ...
    private void doBomb() {
        ...
        if (render.get()) bedRender = new Renderers.SimpleBedRender(placePos.getPos(), bedRenderTime.get(), placePos.getRotationOffset(), sideColor.get(), lineColor.get(), damageColor.get(), bedFadeTime.get(), placePos.getDamage());
    }
    ...
    private void doAntiTrap() {
        ...
        if (trapMine == null) {
            trapMine = new Interactions.MineInstance(target.getBlockPos().up(2));
            trapMine.init();
        } else {
            if (!trapMine.isValid()) {
                trapMine = null;
    ...
    private void doAntiBurrow() {
        ...
        if (burrowMine == null) {
            burrowMine = new Interactions.MineInstance(target.getBlockPos());
            burrowMine.init();
        } else {
    ...
    private boolean placeCheck(DamageCalculator.BedPlacement placement) {
