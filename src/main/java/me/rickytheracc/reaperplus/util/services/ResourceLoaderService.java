package me.rickytheracc.reaperplus.util.services;
...
public class ResourceLoaderService {
    ...
    public static void bindAssetFromFile(Identifier asset, String fileName) {
        ...
        if (!ReaperPlus.USER_ASSETS.exists()) return;
        ReaperPlus.cached.execute(() -> {
            File[] userAssets = ReaperPlus.USER_ASSETS.listFiles();
    ...
    public static class Resource {
        ...
        public File getAsFile() {return new File(ReaperPlus.ASSETS, this.getFileName());}
        ...
        public void cache() {
            ReaperPlus.cached.execute(() -> {
                try {
                    ...
                    ReaperPlus.log("Downloading asset " + this.name);
                    ReaperPlus.log(outFile.getAbsolutePath());
                    FileUtils.copyInputStreamToFile(is, outFile);
                    is.close();
                } catch (Exception ignored) {
                    ReaperPlus.log("Failed to download asset " + this.name);
                    //e.printStackTrace();
                }
            });
        }
        ...
        public void load() {
            ReaperPlus.cached.execute(() -> {
                ...
                try {
                    ...
                    ReaperPlus.log("Loaded asset " + this.name);
                    is.close();
                } catch (Exception ignored) {
                    ReaperPlus.log("Failed to load asset from cache " + this.name);
                    //e.printStackTrace();
                }
            });
        }
